    ...
    return (
        <div className="flex items-end justify-center gap-3 mb-8">
            {podiumOrder.map((entry, i) => {
                if (!entry) return <div key={`podium-${i}`} className="w-28" />;
                return (
                    <div
                        key={`podium-${i}`}
                    ...
                    >
                        ...
                        <div className="text-sm font-bold text-slate-700 text-center leading-tight">
                            {entry.teamName}
                        </div>
...
export default function RankingsPage() {
    ...
    const filteredHackathonEntries = useMemo(() => {
        ...
        const localForSlug = localEntries.filter(
            (e) => e.hackathonSlug === resolvedSlug,
        );
    ...
    return (
        <div className="min-h-screen" style={{ background: "#f5f3ff" }}>
            ...
            <main className="max-w-4xl mx-auto px-4 py-10">
                <div className="mb-8">
                    <p className="text-xs text-violet-400 uppercase tracking-[0.3em] font-bold mb-2">
                        RANKINGS
                    </p>
                ...
                {loading ? (
                        ...
                        <div className="bg-white border border-violet-100 rounded-2xl p-6 shadow-sm">
                            ...
                            <div className="space-y-3">
                                {Array.from({ length: 5 }).map((_, i) => (
                                    <div
                                        key={i}
                                        className="flex items-center gap-4 py-3 animate-pulse border-b border-slate-50 last:border-0"
                                    >
                    ...
                    <ErrorState
                        description="랭킹 데이터를 불러오는 중 오류가 발생했습니다."
                        onRetry={retry}
                    />
                ) : (
                    <>
                            ...
                            (!hackathonLeaderboard ? (
                                ...
                                <div className="bg-white border border-violet-100 rounded-2xl p-10 text-center shadow-sm">
                                    <div className="flex flex-col items-center gap-2 text-slate-400 text-sm">
                                        ...
                                        <button
                                            onClick={() => setPeriod("all")}
                                            className="mt-1 text-xs text-violet-500 underline"
                                        >
                                            전체 기간으로 보기
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between mb-6">
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500 mt-0.5">
                                                업데이트:{" "}
                                                {new Date(
                                                    hackathonLeaderboard.updatedAt,
                                                ).toLocaleString("ko-KR")}{" "}
                                                ·{" "}
                                                {
                                                    filteredHackathonEntries.length
                                                }
                                                팀
                                            </p>
                                        </div>
                                        <Link
                                            href={`/hackathons/${resolvedSlug}`}
